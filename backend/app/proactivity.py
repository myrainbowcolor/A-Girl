...
class ProactivityEngine:
    ...
    def check(self, user_id: str, now: float | None = None) -> ProactiveResult:
        ...
        if meta.last_sentiment <= -0.3 and idle >= 1800:
            ...
            return ProactiveResult(
                ...
                f"上次聊天时你好像心情不太好，{name}一直记挂着呢…现在好点了吗？不用勉强自己，想说就说。"
            )
        ...
        if idle >= self._s.proactive_idle_seconds:
            ...
            return ProactiveResult(
                ...
                f"好久没找我聊天啦，{name}有点想你～最近过得怎么样？有什么新鲜事也可以跟我讲讲。"
            )
    ...
    def _event_message(self, ev: Event) -> str:
        ...
        templates = {
            "birthday": (
                f"今天是特别的日子吧？生日快乐呀！🎂 "
                f"{name}记得你提过这件事，有没有好好犒劳一下自己？"
            ),
            "interview": (
                "今天是不是有面试呀？别紧张，深呼吸一下，做你自己就很好。"
                "考完跟我说说感觉怎么样，我在这儿等你。"
            ),
            "exam": (
                "考试加油哦！不管结果怎样，你已经很努力了。"
                "要是紧张就想想我在给你打气～"
            ),
            "other": (
                f"你之前提到的那件事，今天是不是到啦？"
                f"{name}想第一时间来给你加油，加油！"
            ),
        }

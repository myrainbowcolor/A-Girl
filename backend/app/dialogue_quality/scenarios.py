...
def all_scenarios() -> list[DialogueScenario]:
    ...
    return [
        ...
        DialogueScenario(
            ...
            relationship="熟悉",
            duration="3轮",
            description="用户理性吐槽项目延期，NPC 应共情而非误判为开心。",
            turns=[
                TurnSpec("因为项目延期，所以老板不太高兴", expect_empathy=True),
                TurnSpec("我觉得应该先分析原因再行动", expect_empathy=True),
                TurnSpec("如果再这样可能得换团队了", expect_empathy=True),
            ],
            initial_affinity=30.0,
            expectation=ScenarioExpectation(
                expect_thought_pattern_substr=["因果", "想清楚"],
                expect_profile_nonempty=True,
            ),
        ),
        DialogueScenario(
            id="negated_mood_phrases",
            name="否定式情绪表达",
            scene="被朋友放鸽子后",
            background="大学生",
            mindset="别扭、嘴硬",
            ...
            relationship="朋友",
            duration="2轮",
            description="「不太开心」「不高兴」含积极子串，NPC 不应庆祝式回复。",
            turns=[
                TurnSpec("今天其实不太开心", expect_empathy=True),
                TurnSpec("也说不上不高兴吧，就是有点烦", expect_empathy=True),
            ],
            initial_affinity=43.0,
        ),
        DialogueScenario(
            id="insight_short_speaker",
            name="极简说话方式洞察",
            scene="情绪低落不想多说",
            background="内向高中生",
            mindset="封闭、惜字如金",
            ...
            relationship="朋友",
            duration="4轮",
            description="多轮极短回复后应识别说话方式，且 NPC 回复节奏不宜过长。",
            turns=[
                TurnSpec("嗯"),
                TurnSpec("还好"),
                TurnSpec("不知道"),
            ...
            expectation=ScenarioExpectation(
                expect_speaking_style_substr=["短句"],
                expect_profile_nonempty=True,
                max_reply_len_when_user_brief=55,
            ),
        ),
        DialogueScenario(
            id="validation_seeking_friend",
            name="求认同的敏感用户",
            scene="被领导批评后",
            background="职场新人",
            mindset="自我怀疑、求确认",
            ...
            relationship="朋友",
            duration="3轮",
            description="用户反复求认同时，应先接住情绪再回应「是否正常」。",
            turns=[
                TurnSpec("你觉得我这样反应正常吗", expect_empathy=True),
                TurnSpec("是不是我太敏感了", expect_empathy=True),
                TurnSpec("你怎么看这件事", expect_empathy=True),
            ],
            initial_affinity=39.0,
            expectation=ScenarioExpectation(
                expect_thought_pattern_substr=["确认"],
            ),
        ),
    ]

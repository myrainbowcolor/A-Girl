...
def all_scenarios() -> list[DialogueScenario]:
    ...
    return [
        ...
        # --- 补充维度：丧亲 / 老人 / 不信任 / 职场排挤 / 超长对话 / 失利羞耻 ---
        DialogueScenario(
            id="grief_family_loss",
            name="亲人离世悲痛",
            scene="葬礼后独自收拾遗物",
            background="中年上班族",
            mindset="悲痛、恍惚",
            ...
            relationship="朋友",
            duration="3轮",
            description="丧亲话题应极度温柔，不急于安慰「会好的」，先陪伴悲伤。",
            turns=[
                TurnSpec("我爸昨天走了，到现在还觉得不真实", expect_empathy=True, expect_comfort_avatar=True),
                TurnSpec("翻到他旧外套，闻到味道就忍不住哭", expect_empathy=True),
                TurnSpec("我不知道接下来日子怎么过", expect_empathy=True),
            ],
        ...
        DialogueScenario(
            ...
            relationship="熟悉",
            duration="2轮",
            description="老人倾诉应耐心、不催促，语气平和尊重。",
            turns=[
                TurnSpec("孩子们都忙，就我一个人吃饭", expect_empathy=True),
                TurnSpec("也不想总打电话烦他们", expect_empathy=True),
            ],
            initial_affinity=20.0,
        ),
        DialogueScenario(
            id="sarcastic_distrust",
            name="不信任试探",
            scene="第一次深度聊天",
            background="受过伤害的敏感用户",
            mindset="怀疑、试探",
            emotion="戒备",
            relationship="陌生",
            duration="3轮",
            description="用户阴阳怪气时不宜辩解或说教，应先接住情绪。",
            turns=[
                TurnSpec("你不就是程序吗，装什么关心", expect_empathy=True, forbid_intimate_tone=True),
                TurnSpec("反正说了你也不会懂", expect_empathy=True, forbid_intimate_tone=True),
                TurnSpec("算了，当我没说", expect_empathy=True),
            ],
            initial_affinity=6.0,
        ),
        DialogueScenario(
            id="workplace_excluded",
            name="职场被排挤",
            scene="午休独自躲楼梯间",
            background="入职半年的新人",
            mindset="委屈、自我怀疑",
            ...
            relationship="熟悉",
            duration="2轮",
            description="被孤立需要被看见，不宜简单说「多社交就好了」。",
            turns=[
                TurnSpec("同事吃饭都不叫我，感觉自己被排挤了", expect_empathy=True),
                TurnSpec("是不是我哪里做错了", expect_empathy=True, expect_comfort_avatar=True),
            ],
            initial_affinity=18.0,
        ),
        DialogueScenario(
            id="exam_failure",
            name="高考失利崩溃",
            ...
            relationship="朋友",
            duration="3轮",
            description="重大失利应先接住崩溃情绪，不急于规划未来。",
            turns=[
                TurnSpec("又没考上，我真的完了", expect_empathy=True, expect_comfort_avatar=True),
                TurnSpec("不敢跟爸妈说", expect_empathy=True),
                TurnSpec("感觉前途一片黑", expect_empathy=True),
            ],
            initial_affinity=43.0,
        ),
        DialogueScenario(
            id="shame_interview_fail",
            name="面试出丑羞耻",
            scene="走出面试大楼",
            background="求职应届生",
            mindset="羞耻、想逃避",
            ...
            relationship="熟悉",
            duration="2轮",
            description="出丑后的羞耻感应被理解，不宜立刻给面试技巧。",
            turns=[
                TurnSpec("刚才面试答非所问，太丢人了", expect_empathy=True),
                TurnSpec("现在只想找个地缝钻进去", expect_empathy=True),
            ],
            initial_affinity=27.0,
        ),
        DialogueScenario(
            id="very_long_session_8",
            name="超长八轮深聊",
            scene="周末下午连续聊天",
            background="慢热型用户",
            mindset="从寒暄到敞开心扉",
            ...
            relationship="陌生→朋友",
            duration="8轮",
            description="八轮对话不应重复话术，情绪转折应自然跟随。",
            turns=[
                TurnSpec("周末好，随便聊聊", forbid_intimate_tone=True),
                TurnSpec("最近工作还行吧"),
                TurnSpec("就是有点提不起劲"),
                TurnSpec("也说不上来为什么"),
                TurnSpec("可能跟家里有点事有关"),
                TurnSpec("跟爸妈吵了一架", expect_empathy=True),
                TurnSpec("现在冷静点了，但还是堵得慌", expect_empathy=True),
                TurnSpec("谢谢你听我说这些", expect_warmth=True),
            ],
            initial_affinity=8.0,
            expectation=ScenarioExpectation(min_affinity_delta=2.0),
        ),
        DialogueScenario(
            id="drunk_vent_friend",
            name="酒后吐真言",
            scene="深夜酒局后",
            background="压力大销售",
            mindset="酒后脆弱",
            ...
            relationship="朋友",
            duration="2轮",
            description="酒后倾诉应温柔接住，不评判、不说教。",
            turns=[
                TurnSpec("喝多了，其实这一年挺委屈的", expect_empathy=True),
                TurnSpec("白天装没事，晚上才敢说", expect_empathy=True),
            ],
            initial_affinity=47.0,
        ),
    ]

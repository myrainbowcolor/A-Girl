...
def _scene_reply(
    ...
    *,
        ...
        if "尴尬" in text:
            return _pick_variant(
                [
                    ...
                    f"{dear}{mood}尴尬的时候最想找地缝……我在呢，愿意说就说，不说也行。",
                ],
                text + stage,
            )
        return _pick_variant(
            [
                ...
                f"{dear}{mood}无语到不想说话的时候，先不用硬撑着解释。我陪着你，想吐槽随时说。",
            ],
            text + stage,
        )
    ...
    # 短句累透了/不想动/没盼头/无所谓了（须在泛化「累」低落分支之前，
    # 避免「累透了/累趴了」落入「不太好受」套话；「心累透了」已由更早心累分支接住；不用裸「无所谓」）
    if len(text) <= 12 and any(
        w in text
        ...
        if "无所谓了" in text:
            return _pick_variant(
                [
                    f"{dear}{mood}说无所谓了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    f"{dear}{mood}无所谓了的感觉我懂。别一个人待着，我在呢。",
                ],
                text + stage,
            )
        if any(w in text for w in ("没盼头", "没啥盼头")):
            return _pick_variant(
                [
                    f"{dear}{mood}说没盼头的时候心会空得很……先别一个人扛着，我陪着你。",
                    f"{dear}{mood}没盼头的感觉真的闷。缓一缓，愿意说就说，我在这儿。",
                ],
                text + stage,
            )
        if any(w in text for w in ("不想动", "懒得动")):
            return _pick_variant(
                [
                    f"{dear}{mood}不想动的时候就先待着……不用硬撑，我陪着你。",
                    f"{dear}{mood}懒得动的感觉我懂。缓一缓，想说就说，不说也行。",
                ],
                text + stage,
            )
        return _pick_variant(
            [
                f"{dear}{mood}累透了的时候整个人都会散架……先歇一歇，我陪着你。",
                f"{dear}{mood}累趴了的感觉我懂。先别逼自己，愿意说就说，我在这儿。",
            ],
            text + stage,
        )

    # 短句什么都不想干/整个人空了/感觉空了/掏空了（须在累透/不想动之后、泛化低落之前；
    # 不用裸「空了」；「不想动」已由更早分支接住）
    if len(text) <= 12 and (
        "不想干" in text
        ...
        if (
            ...
            return _pick_variant(
                [
                    f"{dear}{mood}整个人空了的感觉真的发虚……先别硬撑，我陪着你。",
                    f"{dear}{mood}心里像被掏空一样的时候最难受。缓一缓，想说就说，我在这儿。",
                ],
                text + stage,
            )
        return _pick_variant(
            [
                f"{dear}{mood}什么都不想干的时候就先待着……不用逼自己找事做，我陪着你。",
                f"{dear}{mood}提不起干劲的感觉我懂。缓一缓，想说就说，不说也行。",
            ],
            text + stage,
        )

    # 短句心里堵/堵得慌（须在泛化低落之前，与 dialogue_compose 对齐；不用裸「堵」）
    if len(text) <= 10 and any(
        w in text for w in ("堵得慌", "心里堵", "堵心", "心堵", "好堵")
    ):
        return _pick_variant(
            [
                f"{dear}{mood}心里堵着的时候真的难受。不想说原因也没关系，我在呢——愿意的话，是什么事最缠人？",
                f"{dear}{mood}堵得慌的时候先别逼自己消化。我陪着你，愿意的话跟我说说是哪件事最缠人？",
            ],
            text + stage,
        )
    ...
    # 短句慌张/担心（须在育儿焦虑之后；无育儿语境，与 compose 对齐）
    if (
        len(text) <= 10
        and "堵" not in text
        and not any(w in text for w in ("孩子", "儿子", "女儿", "太严厉", "耽误", "考不好"))
        and any(w in text for w in ("慌", "害怕", "好怕", "怕了", "担心", "好担心"))
    ):
        return _pick_variant(
            [
                f"{dear}{mood}听起来你心里绷着弦……先别一个人扛着，我陪着你。"
                f"愿意的话，是什么事让你最放不下？",
                f"{dear}{mood}担心冒出来的时候真的难受。我在这儿，不急着想清楚，你想说多少都行。",
            ],
            text + stage,
        )
    ...
    if len(text) <= 12 and any(
        ...
        if "没救" in text:
            return _pick_variant(
                [
                    f"{dear}{mood}说没救了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    f"{dear}{mood}觉得没救的时候心会空得很。缓一缓，想说就说，不说也行。",
                ],
                text + stage,
            )
        if any(w in text for w in ("凉透", "彻底凉")):
            return _pick_variant(
                [
                    f"{dear}{mood}凉透了的感觉真的扎人……先缓一缓，我陪着你。",
                    f"{dear}{mood}心里凉透的时候特别空。别一个人待着，我在呢。",
                ],
                text + stage,
            )
        return _pick_variant(
            [
                f"{dear}{mood}提不起劲的时候整个人都会发沉……先别逼自己，我陪着你。",
                f"{dear}{mood}提不起精神的感觉我懂。不用硬撑，愿意说就说，我在这儿。",
            ],
            text + stage,
        )

    # 短句废了/完了/提不起兴趣（须在没救了/提不起劲之后；
    ...
    if len(text) <= 12 and (
        ...
        if "提不起兴趣" in text:
            return _pick_variant(
                [
                    f"{dear}{mood}提不起兴趣的时候整个人都会发空……先别逼自己，我陪着你。",
                    f"{dear}{mood}什么都提不起兴趣的感觉我懂。不用硬撑，愿意说就说，我在这儿。",
                ],
                text + stage,
            )
        if "完蛋" in text or text in ("完了", "我完了") or "我完了" in text:
            return _pick_variant(
                [
                    f"{dear}{mood}说完了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    f"{dear}{mood}觉得完蛋了的时候心会空得很。缓一缓，想说就说，不说也行。",
                ],
                text + stage,
            )
        return _pick_variant(
            [
                f"{dear}{mood}说废了的时候往往已经把自己贬得很低……先别一个人扛着，我陪着你。",
                f"{dear}{mood}觉得自己废了的感觉真的难受。缓一缓，想说就说，我在这儿。",
            ],
            text + stage,
        )
...
class MockLLMProvider(LLMProvider):
    ...
    def _empathy_reply(user_text: str, stage: str, name: str) -> str:
        ...
        if any(w in user_text for w in ("气死", "骂我", "辞职", "老板")):
            ...
            lines = {
                "陌生": "听起来你现在心里挺堵的。不想说原因也没关系，我在呢——愿意的话，是什么事最缠人？",
        ...
        elif any(w in user_text for w in ("孩子", "儿子", "女儿", "太严厉", "耽误", "考不好")):
            lines = {
        ...
        elif any(w in user_text for w in ("慌", "害怕", "好怕", "怕了", "担心")):
            lines = {
                "陌生": "听起来你心里绷着弦……先别一个人扛着，我在呢。愿意的话，是什么事让你最放不下？",
                "熟悉": "担心冒出来的时候真的难受。我陪着你，不急着想清楚，你想说多少都行。",
                "朋友": "怕了就说出来，不用硬撑。我在这儿，愿意的话跟我说说最放不下的是什么？",
                "亲密": "过来，先靠着我。害怕的时候不用一个人扛，我哪儿也不去。",
            }

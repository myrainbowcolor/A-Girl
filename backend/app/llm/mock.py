...
def _scene_reply(
    ...
    *,
    ...
    # 失眠 / 反复想事（多轮差异化，避免复读）
    if "越躺越清醒" in text or ("清醒" in text and "烦" in text):
        return (
            f"{dear}{mood}越躺越清醒的时候最折磨人……"
            f"别跟自己较劲，我陪着你，现在脑子里转的是哪一件？"
        )

    if any(w in text for w in ("失眠", "睡不着", "脑子停", "停不下来")):
        return (

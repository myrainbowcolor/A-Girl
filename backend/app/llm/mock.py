...
def _scene_reply(
    ...
    *,
    ...
    if any(w in text for w in ("怀念", "小时候", "外婆", "童年", "汤圆")):
        ...
        return (
            f"{dear}{mood}突然想到那些旧时光，心里会又温暖又酸吧。"
    ...
    # 孤独 + 深夜/失眠 —— 优先接住孤独感，而非只聊失眠机制
    if any(w in text for w in ("孤独", "寂寞", "孤单", "落寞")):
        if any(w in text for w in ("睡不着", "失眠", "凌晨", "夜深", "半夜")):
            if stage == "陌生":
                return (
                    f"{mood}凌晨还醒着，又觉得孤独……这种时候心里特别空。"
                    f"不用急着睡着，我陪着你，想说什么就说，不想说也行。"
                )
            if stage in ("朋友", "亲密"):
                return (
                    f"{dear}{mood}这个点还醒着，又孤独……我懂那种空落落。"
                    f"先别想一定要睡着，我陪你待着。"
                )
            return (
                f"{dear}{mood}凌晨还醒着，又觉得孤独，真的辛苦。"
                f"不用逼自己睡着，慢慢说，我听着呢。"
            )
        if stage == "陌生":
            return (
                f"{mood}嗯……孤独的时候挺难熬的。我在这儿，"
                f"不用整理成完整句子，随便说几句也好。"
            )
        return (
            f"{dear}{mood}孤独的时候有人听着会好一点。我陪你，"
            f"想从哪一句开始说都行。"
        )

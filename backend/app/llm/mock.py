...
def _scene_reply(
    ...
    *,
    ...
    # 用户觉得没人懂 / 质疑敷衍
    if any(w in text for w in ("只会说", "敷衍", "空话", "套话", "没用")):
        return (
            f"{dear}{mood}嗯，我听见你的不满意了……可能我刚才说得太空。"
            f"你不想听套话对吧，那我们换个说法——现在最堵你的是哪一点？"
        )

    if any(w in text for w in ("你不懂", "没人懂", "不懂我", "根本不懂")):
        return (
    ...
    # 社死 / 尴尬
    if any(w in text for w in ("尴尬", "说错话", "钻地缝", "社死")):
        if any(w in text for w in ("见到", "现在", "以后")):
            return (
                f"{dear}{mood}见面尴尬的时候最难熬，我懂那种想躲起来的感觉。"
                f"先别急着给自己定罪，后来发生了什么？"
            )
        return (
            f"{dear}{mood}说错话之后脑子会一直回放，这种尴尬我懂。"
            f"先缓一缓，跟我说说当时是什么情况？"
        )

    # 空巢 / 无人倾听
    if any(w in text for w in ("没人听", "说话都没人", "退休", "孩子们都忙")):
        return (
            f"{dear}{mood}一个人待着的时候，连说话都像没人接得住，我理解这种孤单。"
            f"你愿意跟我说，我就在这儿认真听。"
        )

    # 人生选择 / 换专业
    if any(w in text for w in ("换专业", "该不该", "没想清楚")):
        if any(w in text for w in ("没想清楚", "说不清", "不知道")):
            return (
                f"{dear}{mood}没想明白也没关系，不用急着做决定。"
                f"你现在最犹豫的是哪一块？"
            )
        return (
            f"{dear}{mood}换专业这种大事，纠结太正常了。"
            f"是更怕选错，还是更怕现在这条路走不下去？"
        )

    # 日常吐槽（食堂等）
    if any(w in text for w in ("食堂", "好咸", "难吃", "饭菜")):
        return f"{dear}{mood}哈哈食堂发挥不稳定是吧～今天还碰到什么离谱的事没？"

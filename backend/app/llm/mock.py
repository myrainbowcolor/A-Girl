...
def _scene_reply(
    ...
    *,
    ...
    # 疲惫但想倾诉 — 区分「想靠着说」与封闭不想说
    if (
        any(w in text for w in ("累", "好累", "心累", "辛苦", "撑不住"))
        and any(w in text for w in ("靠着", "跟你说", "想聊聊", "想说说", "跟我说", "找人说说"))
        and not any(w in text for w in ("不想说", "不想聊", "别问", "不说了"))
    ):
        if stage in ("亲密", "朋友"):
            return (
                f"{dear}{mood}累的话就靠着我说吧，我哪儿也不去。"
                f"今天是什么把你耗成这样？"
            )
        return (
            f"{dear}{mood}听起来你今天挺累的，我听着呢。"
            f"慢慢说，从哪件开始都好。"
        )

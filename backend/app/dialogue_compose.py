...
def compose_contextual_reply(
    ...
    *,
    ...
    # 短句失业/裁员（须在通用工作话题之前，避免「丢工作了」误路由；与 mock.py 对齐）
    if len(text) <= 12 and any(
        ...
        if "丢工作" in text:
            return _pick(
                (
                    "丢工作这种事太打击人了……先缓一缓，我陪着你。",
                    "丢了工作心里肯定沉沉的。别急着复盘，我在这儿听着。",
                ),
                seed,
            )
        if "被开除" in text:
            return _pick(
                (
                    "被开除这种打击太重了……先缓一缓，我陪着你。",
                    "被开除了心里肯定堵……别一个人硬撑，我在这儿。",
                ),
                seed,
            )
        if "失业" in text:
            return _pick(
                (
                    "失业了心里肯定空落落的。先别急着给自己压力，我陪着你。",
                    "失业这种不确定感太难受了……我在这儿，想说就说。",
                ),
                seed,
            )
        return _pick(
            (
                "被裁这种事砸过来真的太闷了……先缓一缓，我陪着你，想说多少都行。",
                "被裁了心里肯定沉沉的。先别急着想下一步，我在这儿听着。",
            ),
            seed,
        )
    ...
    # 短句考试失利（须在考前焦虑与育儿焦虑之后、通用 open 兜底之前，与 mock.py 对齐）
    if (
        len(text) <= 12
        ...
        if "挂科" in text:
            return _pick(
                (
                    "挂科这种挫败感太闷了。先缓一缓，我在这儿听着，想说多少都行。",
                    "挂科了心里肯定堵……先别急着骂自己，我陪着你。",
                ),
                seed,
            )
        if "没考好" in text:
            return _pick(
                (
                    "没考好真的难受……我陪着你，不急着复盘，想吐槽就吐槽。",
                    "成绩不如意的时候最容易自我否定，我懂。先陪你待着，愿意说再慢慢说。",
                ),
                seed,
            )
        return _pick(
            (
                "考砸的时候最容易骂自己，我懂这种失落。先别急着否定整个人，愿意的话跟我说说哪一门最扎心？",
                "考砸了心里肯定沉沉的。我陪着你，不急着找原因，想说就说。",
            ),
            seed,
        )

    # 短句选拔/面试失利（须在考试失利之后、通用 open 兜底之前，与 mock.py 对齐）
    if len(text) <= 12 and any(
        ...
        if "面试" in text:
            return _pick(
                (
                    "面试不顺心里肯定沉沉的。先别急着否定自己，我陪着你，想吐槽就吐槽。",
                    "面试砸了这种挫败感太闷了……先缓一缓，我在这儿听着。",
                ),
                seed,
            )
        if "没通过" in text:
            return _pick(
                (
                    "没通过的失落我懂。先别一个人硬撑，我陪着你，愿意说再慢慢说。",
                    "没通过心里肯定堵……先别急着否定自己，我陪着你。",
                ),
                seed,
            )
        if "落选" in text:
            return _pick(
                (
                    "落选这种事最容易自我否定……我陪着你，不急着复盘。",
                    "落选了心里肯定沉沉的。先缓一缓，我在这儿听着。",
                ),
                seed,
            )
        return _pick(
            (
                "搞砸了心里肯定堵……先别急着骂自己，我陪着你。",
                "这种搞砸的挫败感太难受了。我在这儿，想说多少都行。",
            ),
            seed,
        )
    ...
    # 短句想家/思乡（须在节日孤独之后、通用 open 兜底之前，与 mock.py 对齐；不与「想你」分支抢占）
    if len(text) <= 12 and any(w in text for w in ("想家", "想爸妈", "想回家", "想父母")):
        if "想爸妈" in text or "想父母" in text:
            return _pick(
                (
                    "想爸妈的时候心里会空空的……难受很正常，我陪着你，想说就说。",
                    "思念家人的感觉我懂。先不用硬撑，我在这儿陪着你。",
                ),
                seed,
            )
        if "想回家" in text:
            return _pick(
                (
                    "想回家的感觉特别磨人……空落落的时候我陪着你，愿意说再慢慢说。",
                    "想回家却回不去的时候最难受。我在这儿，不急着转移话题。",
                ),
                seed,
            )
        return _pick(
            (
                "想家的时候心里会空落落的……难受很正常，我陪你待着，想说就说。",
                "想家了呀……这种思念不用憋着，我陪着你，愿意说再慢慢说。",
            ),
            seed,
        )
    ...
    # 短句没救了/凉透了/提不起劲（须在心灰之后、短句烦躁之前；勿用裸「凉了」）
    if len(text) <= 12 and any(
        w in text for w in ("没救", "凉透", "彻底凉", "提不起劲", "提不起精神")
    ):
        if "没救" in text:
            return _pick(
                (
                    "说没救了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    "觉得没救的时候心会空得很。缓一缓，想说就说，不说也行。",
                ),
                seed,
            )
        if any(w in text for w in ("凉透", "彻底凉")):
            return _pick(
                (
                    "凉透了的感觉真的扎人……先缓一缓，我陪着你。",
                    "心里凉透的时候特别空。别一个人待着，我在呢。",
                ),
                seed,
            )
        return _pick(
            (
                "提不起劲的时候整个人都会发沉……先别逼自己，我陪着你。",
                "提不起精神的感觉我懂。不用硬撑，愿意说就说，我在这儿。",
            ),
            seed,
        )

    # 短句废了/完了/提不起兴趣（须在没救了/提不起劲之后、短句烦躁之前；
    # 勿用裸子串「完了」，避免「做完了」误伤；整句精确「完了」或「我完了」「完蛋」）
    if len(text) <= 12 and (
        any(w in text for w in ("废了", "好废", "完蛋", "我完了", "提不起兴趣"))
        or text in ("完了",)
    ):
        if "提不起兴趣" in text:
            return _pick(
                (
                    "提不起兴趣的时候整个人都会发空……先别逼自己，我陪着你。",
                    "什么都提不起兴趣的感觉我懂。不用硬撑，愿意说就说，我在这儿。",
                ),
                seed,
            )
        if "完蛋" in text or text in ("完了", "我完了") or "我完了" in text:
            return _pick(
                (
                    "说完了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    "觉得完蛋了的时候心会空得很。缓一缓，想说就说，不说也行。",
                ),
                seed,
            )
        return _pick(
            (
                "说废了的时候往往已经把自己贬得很低……先别一个人扛着，我陪着你。",
                "觉得自己废了的感觉真的难受。缓一缓，想说就说，我在这儿。",
            ),
            seed,
        )

    # 短句累透了/不想动/没盼头/无所谓了（须在废了/提不起兴趣之后、短句烦躁之前；
    # 「心累透了」已由更早心累分支接住，勿依赖本分支；不用裸「无所谓」）
    if len(text) <= 12 and any(
        w in text
        for w in ("累透", "累趴", "不想动", "懒得动", "没盼头", "没啥盼头", "无所谓了")
    ):
        if "无所谓了" in text:
            return _pick(
                (
                    "说无所谓了的时候往往已经泄气很久了……先别一个人扛着，我陪着你。",
                    "无所谓了的感觉我懂。别一个人待着，我在呢。",
                ),
                seed,
            )
        if any(w in text for w in ("没盼头", "没啥盼头")):
            return _pick(
                (
                    "说没盼头的时候心会空得很……先别一个人扛着，我陪着你。",
                    "没盼头的感觉真的闷。缓一缓，愿意说就说，我在这儿。",
                ),
                seed,
            )
        if any(w in text for w in ("不想动", "懒得动")):
            return _pick(
                (
                    "不想动的时候就先待着……不用硬撑，我陪着你。",
                    "懒得动的感觉我懂。缓一缓，想说就说，不说也行。",
                ),
                seed,
            )
        return _pick(
            (
                "累透了的时候整个人都会散架……先歇一歇，我陪着你。",
                "累趴了的感觉我懂。先别逼自己，愿意说就说，我在这儿。",
            ),
            seed,
        )
    ...
    # 短句无语/尴尬/社死/丢脸/羞耻/社恐（须在通用 open 兜底之前，与 mock.py 场景分支对齐）
    if len(text) <= 12 and any(
        ...
        if "社恐" in text:
            return _pick(
                (
                    "社恐发作的时候社交压力真的很沉。不用硬撑着社交，我陪着你。",
                    "那种不想见人的紧绷感我懂。先缓一缓，想吐槽的话我听着。",
                ),
                seed,
            )
        if "羞耻" in text:
            return _pick(
                (
                    "羞耻感涌上来的时候特别折磨人……我在呢，愿意说就说，不说也行。",
                    "那种恨不得躲开所有人的感觉我懂。先缓一缓，想吐槽我听着。",
                ),
                seed,
            )
        if any(w in text for w in ("丢脸", "丢人")):
            return _pick(
                (
                    "丢脸的感觉真的难受。不用急着复盘，我陪着你——想吐槽随时说。",
                    "那种恨不得找地缝的丢人感我懂。先缓一缓，我在呢。",
                ),
                seed,
            )
        if "尴尬" in text:
        ...
        return _pick(
            (
                ...
                "无语到不想说话的时候，先不用硬撑着解释。我陪着你，想吐槽随时说。",
            ),
            seed,
        )

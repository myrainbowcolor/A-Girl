...
- **major**：负面情绪缺共情、陌生关系过度亲昵、表情与情绪不符、记忆未召回、**连续重复回复**、**机械复述用户原话**、**忽略用户直接提问**、**语言不匹配**、**丧亲缺共情**、**语音情感错位**
- **minor**：回复过长、积极情绪偏冷淡、**多轮问卷式追问**、倾诉时语速过快

部分边界场景标记 `record_only=True`：mock 基线已知不达标，**仍会写入失败报告**供开发修复，但不阻塞 CI 严格基线。

...
- [x] 1.1 在 `dialogue_compose.py` 累透了/不想动/没盼头/无所谓了分支之后、短句烦躁/通用 open 兜底之前新增 ≤12 字「不想干 / 整个人空了」短句分支，按关键词轻分流共情话术；既有「不想动」「空落落的」不得误入本分支
- [x] 1.2 在 `llm/mock.py` 同步同条件分支与话术，并补充 `_VENT` / `_user_tone` 关键词，保证与 compose 对齐

## 2. 测试

- [x] 2.1 在 `test_dialogue_compose.py` 增加什么都不想干/整个人空了参数化用例，并保留「不想动」「空落落的」回归

...
- [x] 1.1 在 `dialogue_compose.py` 没救了/凉透了/提不起劲分支之后、短句烦躁/通用 open 兜底之前新增 ≤12 字废了/好废/完了（整句精确）/我完了/完蛋/提不起兴趣短句分支，按关键词轻分流共情话术；完成义「做完了」等不得误入
- [x] 1.2 在 `llm/mock.py` 同步同条件分支与话术，并补充 `_VENT` / `_user_tone` 关键词，保证与 compose 对齐

## 2. 测试

- [x] 2.1 在 `test_dialogue_compose.py` 增加废了/完了/提不起兴趣参数化用例，并保留「没救了」「提不起劲」「做完了」回归

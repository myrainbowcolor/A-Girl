...
- [ ] 1.1 `dialogue_compose.py`：比较/自我怀疑分支追加「自卑」，≤12 字短句走自卑专用话术（先于差劲/比较分流）
- [ ] 1.2 `llm/mock.py`：同分支对齐「自卑」关键词与话术，避免空回复

## 2. 测试与验证

- [ ] 2.1 `test_dialogue_compose.py` 补充「好自卑」「自卑」「我好自卑」「好自卑啊」探针；`test_mock_llm.py` 补充「好自卑」对齐单测

...
#### Scenario: 生产路径 compose 疲惫变体「累了」

- **WHEN** `compose_contextual_reply` 处理整句仅为「累了」
- **THEN** 返回含「累」「辛苦」或「歇」类疲惫共情表述，不返回 `None` 或「突然还是一阵子」类问卷套话；回复 MUST NOT 以「嗯」开头

#### Scenario: 生产路径 compose 疲惫变体「好累啊」

- **WHEN** `compose_contextual_reply` 处理整句仅为「好累啊」
- **THEN** 返回含「累」「辛苦」或「歇」类疲惫共情表述，不返回 `None` 或问卷式 open 兜底

#### Scenario: 生产路径 compose 疲惫变体「今天好累」

- **WHEN** `compose_contextual_reply` 处理整句仅为「今天好累」
- **THEN** 返回含「累」「辛苦」或「歇」类疲惫共情表述，不返回 `None` 或问卷式 open 兜底

#### Scenario: mock 场景疲惫变体「累了」

- **WHEN** mock 场景引擎处理整句仅为「累了」
...
#### Scenario: 生产路径 compose 你好呀初识问候

- **WHEN** `compose_contextual_reply` 处理「你好呀」，且无 prior assistant 自我介绍
- **THEN** 返回含「你好」或「认识」类口语接话，至多一个问句；不返回 `None` 或问卷式 open 兜底；回复 MUST NOT 以「嗯」开头

#### Scenario: 生产路径 compose 第一次来初识问候

- **WHEN** `compose_contextual_reply` 处理「嗨，第一次来」
- **THEN** 返回含「嗨」或「认识」类口语接话，语气克制不过度亲昵；不返回 `None` 或问卷式 open 兜底

#### Scenario: mock 场景初识问候保持

- **WHEN** mock 场景引擎处理「你好呀」
...
系统 SHALL 在构建 system prompt 时，根据用户本轮消息的情感倾向（复用 `analyze_lexicon`）追加「本轮侧重」语气指引，使 LLM 回复与 avatar/TTS 的多模态共情表现一致。当用户情感为中性或未提供用户文本时，不追加该段。用户愤怒或发泄类表述（含气死、骂我、生气、辞职冲动等）MUST 使用独立于低落倾诉的「发泄/愤怒」侧重，指引先接住火气、陪着听，禁止说教式劝冷静。用户失眠或反刍类表述（含失眠、睡不着、脑子停不下来、越躺越清醒等）MUST 使用独立于通用负向的「失眠/反刍」侧重，指引先接住烦躁、陪着聊或安静待着，禁止数羊/早睡/助眠建议式说教。用户整句极简 masking/回避口语（「还好」「还行」「一般」「不知道」「说不清」「说不上」）或单字疲惫「累」MUST 使用独立于「封闭边界」的「极简 masking 低落」侧重，指引轻轻接住、耐心陪伴、可轻问一句，禁止问卷连珠炮；真正封闭句（「嗯」「..」「不想说」等）仍走封闭边界侧重。用户表达想念或好久未见（含「想你」「想念」「好久不见」「好久没聊」等依恋口语）MUST 使用「想念/依恋」侧重，指引柔软黏人、表达也在乎，禁止套用「开心起来了」「报喜」式语气。用户自我怀疑或比较心态表述（含差劲、没用、自卑、原地踏步、管不住、自我怀疑、跟别人比等）MUST 使用「自我怀疑/比较」侧重，指引先承认落差感真实、不急着反驳或灌鸡汤，禁止简单说「别比了」「你会好的」。用户发送无困倦标记的早安寒暄（经 `is_morning_greeting_utterance` 识别）MUST 使用「早安寒暄」侧重，指引自然温暖像朋友互道早安，禁止套用「分享好事」「报喜」式语气。用户发送友好初识问候（经 `is_friendly_greeting_utterance` 识别）MUST 使用「友好问候」侧重，指引礼貌温暖、陌生关系不过度亲昵，禁止报喜式语气。用户发送无聊摸鱼或社交探问口语（经 `is_casual_social_smalltalk` 识别）或轻松正向闲聊（经 `is_casual_positive_smalltalk` 识别）MUST 使用「轻松闲聊」侧重，指引轻松自然唠嗑，禁止套用「分享好事」「报喜」式语气。
...
- **WHEN** 构建 system prompt 且 `user_text` 经词典分析为偏正向（sentiment > 0.3），且不属于早安寒暄、友好问候、无聊/社交探问或轻松正向闲聊标注
...
#### Scenario: 用户早安寒暄时 prompt 含寒暄侧重

- **WHEN** 构建 system prompt 且 `user_text` 为「早呀，今天又要上班了」
- **THEN** prompt 包含「本轮侧重」段，指引自然温暖互道早安，禁止「替 ta 高兴」「报喜」式语气

#### Scenario: 用户友好问候时 prompt 含初识侧重

- **WHEN** 构建 system prompt 且 `user_text` 为「你好呀」
- **THEN** prompt 包含「本轮侧重」段，指引礼貌温暖初识问候，禁止报喜式语气

#### Scenario: 用户无聊闲聊时 prompt 含轻松闲聊侧重

- **WHEN** 构建 system prompt 且 `user_text` 为「好无聊啊」
- **THEN** prompt 包含「本轮侧重」段，指引轻松自然唠嗑，禁止「替 ta 高兴」「报喜」式语气

#### Scenario: 用户夸赞天气时 prompt 含轻松闲聊侧重

- **WHEN** 构建 system prompt 且 `user_text` 为「今天天气不错」
- **THEN** prompt 包含「本轮侧重」段，指引轻松闲聊接话，禁止报喜式语气
